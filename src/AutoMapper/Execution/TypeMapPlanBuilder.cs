namespace AutoMapper.Execution
{
    ...
    {
        public static LambdaExpression BuildMapperFunc(TypeMap typeMap, TypeMapRegistry typeMapRegistry)
        {
            ...
            {
                return Lambda(typeMap.Substitution.ReplaceParameters(srcParam, destParam, ctxtParam), srcParam, destParam, ctxtParam);
            }
            ...
            if (typeMap.TypeConverterType != null)
            {
                ...
                Type converterInterfaceType = typeof(ITypeConverter<,>).MakeGenericType(typeMap.SourceType, typeMap.DestinationType);
                return Lambda(
                    Call(
                        Convert(
                            Call(
            ...
            {
                return Lambda(typeMap.CustomMapper.ReplaceParameters(srcParam, destParam, ctxtParam), srcParam, destParam, ctxtParam);
            }
            ...
            var assignmentFunc = CreateAssignmentFunc(typeMap, typeMapRegistry, srcParam, destParam, ctxtParam, destinationFunc);
            ...
            Expression assignmentFunc)
        {
            ...
            if (typeMap.PreserveReferences)
            {
                ...
                var condition = Condition(
                    ...
                    Assign(cache, ToType(Property(Property(ctxtParam, "InstanceCache"), "Item", srcParam), typeMap.DestinationType)),
        ...
        {
            var assignTypeMap = Assign(MakeMemberAccess(ctxtParam, typeof(ResolutionContext).GetProperty("TypeMap")), Constant(typeMap));

            var beforeMap = Call(ctxtParam, typeof(ResolutionContext).GetMethod("BeforeMap"), ToObject(destParam));

            var typeMaps =
                typeMap.GetPropertyMaps().Where(pm => pm.CanResolveValue()).Select(pm => TryPropertyMap(pm, registry, srcParam, destParam, ctxtParam)).ToList();
            ...
            actions.AddRange(typeMap.AfterMapActions.Select(afterMapAction => afterMapAction.ReplaceParameters(srcParam, destParam, ctxtParam)));
        ...
        private static Expression TryPropertyMap(
            ...
            ParameterExpression ctxtParam)
        {
            var pmExpression = CreatePropertyMapFunc(pm, registry, srcParam, destParam, ctxtParam);
            ...
            var mappingExceptionCtor = typeof(AutoMapperMappingException).GetTypeInfo().DeclaredConstructors.First(ci => ci.GetParameters().Length == 3);
            ...
            return TryCatch(Block(typeof(void), pmExpression),
                ...
                MakeCatchBlock(typeof(Exception), exception, Throw(New(mappingExceptionCtor, ctxtParam, exception, Constant(pm))), null));
        }
        ...
        private static Expression CreateDestinationFunc(
            ...
            ParameterExpression ctxtParam)
        {
            var newDestFunc = ToType(CreateNewDestinationFunc(typeMap, typeMapRegistry, srcParam, ctxtParam), typeMap.DestinationType);
            ...
            ParameterExpression ctxtParam)
        {
            ...
            if (typeMap.ConstructDestinationUsingServiceLocator)
                return Call(MakeMemberAccess(ctxtParam, typeof(ResolutionContext).GetProperty("Options")),
                                typeof(MappingOperationOptions).GetMethod("CreateInstance").MakeGenericMethod(typeMap.DestinationType)
                                );
