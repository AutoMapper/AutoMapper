...
{
    public class ConvertMapperTests
    {
        [Fact]
        public void Should_convert_primitive_types()
        {
            var convertMapper = new ConvertMapper();

            foreach(var typePair in _converters.Keys)
            {
                convertMapper.IsMatch(typePair).ShouldBeTrue();
            }
        }

        IDictionary<TypePair, LambdaExpression> _converters = new Dictionary<TypePair, LambdaExpression>
        {
			// bool
